...
void TestBlock::addTransaction(TestTransaction const& _tr)
{
    ...
    if (m_transactionQueue.import(_tr.transaction().rlp()) != ImportResult::Success)
        LOGINF << TestOutputHelper::get().testName() + " Test block failed importing transaction\n";
    recalcBlockHeaderBytes();
}
...
void TestBlock::mine(TestBlockChain const& _bc)
{
    ...
    try
    {
        ...
        // mining!");
        LOGINF << "Mining block difficulty of:  " << block.info().difficulty();
        ...
        if (transactionsOnImport > block.pending().size())
            LOGINF << TestOutputHelper::get().testName() +
    ...
    LOGINF << "Mined TrRoot: " << m_blockHeader.transactionsRoot();
...
BlockHeader TestBlock::constructBlock(mObject const& _o, h256 const& _stateRoot)
{
    ...
    try
    {
        ...
        // LOGDBG << "Block constructed of hash" << ret.hash() << "(without:" <<
        // ret.hash(WithoutSeal) << ")";
    }
    catch (Exception const& _e)
    {
        LOGINF << TestOutputHelper::get().testName() + " block population did throw an exception: "
               << diagnostic_information(_e);
    }
    catch (std::exception const& _e)
    {
...
void TestBlockChain::reset(TestBlock const& _genesisBlock, MiningType _mining)
{
    ...
    {
        LOGDBG << "Not known:" << BlockHeader::headerHashFromBlock(_genesisBlock.bytes())
               << BlockHeader(p.genesisBlock()).hash();
        LOGDBG << "Genesis block not known!";
        LOGDBG << "This should never happen.";
        assert(false);
    }
...
vector<TestBlock> TestBlockChain::syncUncles(vector<TestBlock> const& uncles)
{
    ...
    for (size_t i = 0; i < uncles.size(); i++)
    {
        ...
        {
            LOGINF << "error in importing uncle! This produces an invalid block (May be by purpose "
                      "for testing).";
        }
    }
