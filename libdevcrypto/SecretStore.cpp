	...
	try
	{
		...
		if (u.type() == js::obj_type)
		{
			...
			if (o.find("address") != o.end() && isHex(o["address"].get_str()))
				...
				LOGWRN << "Account address is either not defined or not in hex format" << _file.string();
			m_keys[uuid] = EncryptedKey{js::write_string(o["crypto"], false), _file, address};
			return uuid;
		}
		else
			LOGWRN << "Invalid JSON in key file" << _file.string();
		return h128();
	}
	catch (...)
	{
...
bytesSec SecretStore::decrypt(string const& _v, string const& _pass)
{
	...
	if (o["kdf"].get_str() == "pbkdf2")
	{
		...
		{
			LOGWRN << "Unknown PRF for PBKDF2" << params["prf"].get_str() << "not supported.";
			return bytesSec();
		}
	...
	{
		LOGWRN << "Unknown KDF" << o["kdf"].get_str() << "not supported.";
		return bytesSec();
	}

	if (derivedKey.size() < 32 && !(o.count("compat") && o["compat"].get_str() == "2"))
	{
		LOGWRN << "Derived key's length too short (<32 bytes)";
		return bytesSec();
	}
	...
	if (o.count("mac"))
	{
		...
		if (mac != macExp)
		{
			LOGWRN << "Invalid key - MAC mismatch; expected" << toString(macExp) << ", got" << toString(mac);
			return bytesSec();
		}
	}
	else if (o.count("sillymac"))
	{
		...
		LOGWRN << "No MAC. Proceeding anyway.";
	...
	{
		LOGWRN << "Unknown cipher" << o["cipher"].get_str() << "not supported.";
		return bytesSec();
	}
}
