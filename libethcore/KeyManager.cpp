    ...
    try
    {
        ...
        if (version == 1)
        {
            ...
            for (auto const& i : s[1])
            {
                ...
                // LOGDBG << toString(addr) << toString(uuid) << toString((h256)i[2])<<(string)i[3];
            }
            if (saveRequired)
                m_store.save();
        ...
        //		LOGDBG << hashPassword(m_password) << toHex(m_password);
        cachePassword(m_defaultPasswordDeprecated);
        //		LOGDBG << hashPassword(asString(m_key.ref())) << m_key.hex();
        cachePassword(asString(m_keysFileKey.ref()));
        //		LOGDBG << hashPassword(_pass) << _pass;
