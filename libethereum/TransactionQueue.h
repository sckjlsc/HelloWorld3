...
private:
    ...
    struct PriorityCompare
    {
        ...
        bool operator()(VerifiedTransaction const& _first, VerifiedTransaction const& _second) const
        {
            ...
            return height1 < height2 || (height1 == height2 && _first.transaction.gasPrice() >
                                                                   _second.transaction.gasPrice());
        }
    };
    ...
    std::unordered_map<h256, PriorityQueue::iterator> m_currentByHash;  ///< Transaction hash to set
                                                                        ///< ref
    ...
    Signal<ImportResult, h256 const&, h512 const&> m_onImport;  ///< Called for each import attempt.
                                                                ///< Arguments are result,
                                                                ///< transaction id an node id. Be
                                                                ///< nice and exit fast.
    ...
    Logger m_tq{createLogger(VerbosityInfo, "tq")};
    Logger m_tq_dbg{createLogger(VerbosityDebug, "tq")};

    inline std::string tq_location(const std::string& path) const
    {
        return path.substr(path.find_last_of("/\\") + 1);
    }

#define LOGTQINF LOG(m_tq) << "[" << tq_location(__FILE__) << ":" << __LINE__ << "] "
#define LOGTQDBG LOG(m_tq_dbg) << "[" << tq_location(__FILE__) << ":" << __LINE__ << "] "
};
