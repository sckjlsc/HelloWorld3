...
private:
    ...
    Logger m_blk_logger{createLogger(VerbosityDebug, "block")};
    Logger m_blk_loggerDetailed{createLogger(VerbosityTrace, "block")};

    inline std::string blk_location(const std::string& path) const
    {
        return path.substr(path.find_last_of("/\\") + 1);
    }

#define LOGBLKTRC \
    LOG(m_blk_loggerDetailed) << "[" << blk_location(__FILE__) << ":" << __LINE__ << "] "
#define LOGBLKDBG LOG(m_blk_logger) << "[" << blk_location(__FILE__) << ":" << __LINE__ << "] "
};
