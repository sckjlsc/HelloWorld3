 ...
 * Syncs to peers and keeps up to date. Base class handles blocks downloading but does not contain
 * any details on state transfer logic.
 */
...
private:
    ...
    std::set<std::weak_ptr<EthereumPeer>, std::owner_less<std::weak_ptr<EthereumPeer>>>
        m_daoChallengedPeers;  ///> Peers to which we've sent DAO challenge request
    ...
    std::unordered_set<unsigned> m_downloadingHeaders;  ///< Set of block body numbers being
                                                        ///< downloaded
    std::unordered_set<unsigned> m_downloadingBodies;   ///< Set of block header numbers being
                                                        ///< downloaded
    ...
    std::map<std::weak_ptr<EthereumPeer>, std::vector<unsigned>,
        std::owner_less<std::weak_ptr<EthereumPeer>>>
        m_headerSyncPeers;  ///< Peers to m_downloadingSubchain number map
    std::map<std::weak_ptr<EthereumPeer>, std::vector<unsigned>,
        std::owner_less<std::weak_ptr<EthereumPeer>>>
        m_bodySyncPeers;  ///< Peers to m_downloadingSubchain number map
    std::unordered_map<HeaderId, unsigned, HeaderIdHash> m_headerIdToNumber;
    bool m_haveCommonHeader =
        false;  ///< True if common block for our and remote chain has been found
    ...
    Logger m_bsc_dbg{createLogger(VerbosityDebug, "sync")};
    Logger m_bcs_inf{createLogger(VerbosityInfo, "sync")};
    Logger m_bcs_trc{createLogger(VerbosityTrace, "sync")};
    inline std::string bcs_location(const std::string& path)
    {
        return path.substr(path.find_last_of("/\\") + 1);
    }

#define LOGBCSTRC LOG(m_bcs_trc) << "[" << bcs_location(__FILE__) << ":" << __LINE__ << "] "
#define LOGBCSDBG LOG(m_bsc_dbg) << "[" << bcs_location(__FILE__) << ":" << __LINE__ << "] "
#define LOGBCSINF LOG(m_bcs_inf) << "[" << bcs_location(__FILE__) << ":" << __LINE__ << "] "
