...
{
    // LOGINF << "startWorking for thread" << m_name;
    ...
    else
    {
        ...
        m_work.reset(new thread([&]() {
            ...
            while (m_state != WorkerState::Killing)
            {
                ...
                // LOGINF << "Trying to set Started: Thread was" << (unsigned)ex << "; "<< ok;
                m_state_notifier.notify_all();
                ...
                {
                    LOGWRN << "Exception thrown in Worker thread: " << _e.what();
                }
...

                {
                    ...
                    //					LOGINF << "State: Stopped: Thread was" << (unsigned)ex;
                    if (ex == WorkerState::Killing || ex == WorkerState::Starting)
                        m_state.exchange(ex);
                }
                m_state_notifier.notify_all();
                // LOGINF << "Waiting until not Stopped...";
...
{
    // LOGINF << "stopWorking for thread" << m_name;
    std::unique_lock<Mutex> l(x_work);
    if (m_work)
    {
