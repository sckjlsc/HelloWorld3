...
protected:
    /// Called by NodeTable on behalf of an implementation (Host) to process new events without
    /// blocking nodetable.
    void processEvents()
    {
 ...
 *
 * [Protocol]
 * @todo optimize knowledge at opposite edges; eg, s_bitsPerStep lookups. (Can be done via pointers
 * to NodeBucket)
...
public:
    ...
    /// Called by implementation which provided handler to process NodeEntryAdded/NodeEntryDropped
    /// events. Events are coalesced by type whereby old events are ignored.
    void processEvents();

    /// Add node. Node will be pinged and empty shared_ptr is returned if node has never been seen
    /// or NodeID is empty.
    std::shared_ptr<NodeEntry> addNode(
        Node const& _node, NodeRelation _relation = NodeRelation::Unknown);
...
#if defined(BOOST_AUTO_TEST_SUITE) || \
    defined(_MSC_VER)  // MSVC includes access specifier in symbol name
protected:
#else
private:
#endif
    ...
    static unsigned const s_maxSteps =
        boost::static_log2<s_bits>::value;  ///< Max iterations of discovery. (discover)

    /// Chosen constants

    static unsigned const s_bucketSize =
        16;  ///< Denoted by k in [Kademlia]. Number of nodes stored in each bucket.
    static unsigned const s_alpha =
        3;  ///< Denoted by \alpha in [Kademlia]. Number of concurrent FindNode requests.

    /// Intervals

    /* todo: replace boost::posix_time; change constants to upper camelcase */
    std::chrono::milliseconds const c_evictionCheckInterval =
        std::chrono::milliseconds(75);  ///< Interval at which eviction timeouts are checked.
    std::chrono::milliseconds const c_reqTimeout = std::chrono::milliseconds(
        300);  ///< How long to wait for requests (evict, find iterations).
    std::chrono::milliseconds const c_bucketRefresh = std::chrono::milliseconds(
        7200);  ///< Refresh interval prevents bucket from becoming stale. [Kademlia]
    ...
    /// Returns center node entry which describes this node and used with dist() to calculate xor
    /// metric for node table nodes.
    NodeEntry center() const { return NodeEntry(m_node.id, m_node.publicKey(), m_node.endpoint); }

    /// Used by asynchronous operations to return NodeEntry which is active and managed by node
    /// table.
    std::shared_ptr<NodeEntry> nodeEntry(NodeID _id);

    /// Used to discovery nodes on network which are close to the given target.
    /// Sends s_alpha concurrent requests to nodes nearest to target, for nodes nearest to target,
    /// up to s_maxSteps rounds.
    void doDiscover(NodeID _target, unsigned _round = 0,
        std::shared_ptr<std::set<std::shared_ptr<NodeEntry>>> _tried =
            std::shared_ptr<std::set<std::shared_ptr<NodeEntry>>>());
    ...
    /// Asynchronously drops _leastSeen node if it doesn't reply and adds _new node, otherwise _new
    /// node is thrown away.
    ...
    /// Called by evict() to ensure eviction check is scheduled to run and terminates when no
    /// evictions remain. Asynchronous.
    ...
    Node m_node;  ///< This node. LOCK x_state if endpoint access or mutation is required. Do not
                  ///< modify id.
    Secret m_secret;  ///< This nodes secret key.

    mutable Mutex x_nodes;  ///< LOCK x_state first if both locks are required. Mutable for
                            ///< thread-safe copy in nodes() const.
    ...
    Mutex x_evictions;  ///< LOCK x_evictions first if both x_nodes and x_evictions locks are
                        ///< required.
    std::unordered_map<NodeID, EvictionTimeout> m_evictions;  ///< Eviction timeouts.

    Mutex x_pubkDiscoverPings;  ///< LOCK x_nodes first if both x_nodes and x_pubkDiscoverPings
                                ///< locks are required.
    std::unordered_map<bi::address, TimePoint> m_pubkDiscoverPings;  ///< List of pending pings
                                                                     ///< where node entry wasn't
                                                                     ///< created due to unkown
                                                                     ///< pubk.

    Mutex x_findNodeTimeout;
    std::list<NodeIdTimePoint> m_findNodeTimeout;  ///< Timeouts for FindNode requests.

    std::shared_ptr<NodeSocket> m_socket;  ///< Shared pointer for our UDPSocket; ASIO requires
                                           ///< shared_ptr.
    NodeSocket* m_socketPointer;  ///< Set to m_socket.get(). Socket is created in constructor and
                                  ///< disconnected in destructor to ensure access to pointer is
                                  ///< safe.

    Logger m_nt_dbg{createLogger(VerbosityDebug, "discov")};
    inline std::string nt_location(const std::string& path) const
    {
        return path.substr(path.find_last_of("/\\") + 1);
    }

#define LOGNTDBG LOG(m_nt_dbg) << "[" << nt_location(__FILE__) << ":" << __LINE__ << "] "
...
struct DiscoveryDatagram : public RLPXDatagramFace
{
    ...
    DiscoveryDatagram(bi::udp::endpoint const& _to)
      : RLPXDatagramFace(_to), ts(futureFromEpoch(std::chrono::seconds(60)))
    {}
...
{
    Neighbours(bi::udp::endpoint _to, std::vector<std::shared_ptr<NodeEntry>> const& _nearest,
        unsigned _offset = 0, unsigned _limit = 0)
      : DiscoveryDatagram(_to)
    {
        auto limit =
            _limit ? std::min(_nearest.size(), (size_t)(_offset + _limit)) : _nearest.size();
