...
{
    LOGHSTRC << "p2p.connect.egress sending auth to " << m_socket->remoteEndpoint();
    ...
    // E(remote-pubk, S(ecdhe-random, ecdh-shared-secret^nonce) || H(ecdhe-random-pubk) || pubk ||
    // nonce || 0x0)
    Secret staticShared;
...
{
    LOGHSTRC << "p2p.connect.ingress sending ack to " << m_socket->remoteEndpoint();
...
{
    LOGHSTRC << "p2p.connect.ingress sending EIP-8 ack to " << m_socket->remoteEndpoint();
...
void RLPXHandshake::setAuthValues(Signature const& _sig, Public const& _remotePubk,
...
{
    LOGHSTRC << "p2p.connect.ingress receiving auth from " << m_socket->remoteEndpoint();
    m_authCipher.resize(307);
    auto self(shared_from_this());
    ba::async_read(m_socket->ref(), ba::buffer(m_authCipher, 307),
...
void RLPXHandshake::readAuthEIP8()
{
    ...
    LOGHSTRC << "p2p.connect.ingress receiving " << size << " bytes EIP-8 auth from "
    ...
    ba::async_read(m_socket->ref(), rest, [this, self](boost::system::error_code ec, std::size_t) {
        ...
        {
            LOGHSTRC << "p2p.connect.ingress auth decrypt failed for "
                     << m_socket->remoteEndpoint();
            m_nextState = Error;
            transition();
        }
...
{
    LOGHSTRC << "p2p.connect.egress receiving ack from " << m_socket->remoteEndpoint();
...
void RLPXHandshake::readAckEIP8()
{
    ...
    LOGHSTRC << "p2p.connect.egress receiving " << size << " bytes EIP-8 ack from "
             << m_socket->remoteEndpoint();
    ...
    ba::async_read(m_socket->ref(), rest, [this, self](boost::system::error_code ec, std::size_t) {
        ...
        {
            LOGHSTRC << "p2p.connect.egress ack decrypt failed for " << m_socket->remoteEndpoint();
            m_nextState = Error;
            transition();
        }
    });
...
void RLPXHandshake::error()
{
    ...
    if (connected && !m_socket->remoteEndpoint().address().is_unspecified())
        LOGHSTRC << "Disconnecting " << m_socket->remoteEndpoint() << " (Handshake Failed)";
    else
        LOGHSTRC << "Handshake Failed (Connection reset by peer)";
...
void RLPXHandshake::transition(boost::system::error_code _ech)
{
    ...
    {
        LOGHSTRC << "Handshake Failed (I/O Error: " << _ech.message() << ")";
        return error();
    }
    ...
    m_idleTimer.async_wait([this, self](boost::system::error_code const& _ec) {
        if (!_ec)
        {
            if (!m_socket->remoteEndpoint().address().is_unspecified())
                LOGHSTRC << "Disconnecting " << m_socket->remoteEndpoint()
                         << " (Handshake Timeout)";
            cancel();
        }
    });
    ...
    else if (m_nextState == WriteHello)
    {
        ...
        LOGHSTRC << (m_originated ? "p2p.connect.egress" : "p2p.connect.ingress")
        ...
        s.append((unsigned)HelloPacket).appendList(5)
            << dev::p2p::c_protocolVersion << m_host->m_clientVersion << m_host->caps()
        ...
        ba::async_read(m_socket->ref(), boost::asio::buffer(m_handshakeInBuffer, handshakeSize),
            [this, self](boost::system::error_code ec, std::size_t) {
                ...
                else
                {
                    if (!m_io)
                    {
                        LOGHSTRC << "Internal error in handshake: RLPXFrameCoder disappeared.";
                        m_nextState = Error;
                        transition();
                        return;
                    }
                    ...
                    ba::async_read(m_socket->ref(),
                        ...
                        [this, self, headerRLP](boost::system::error_code ec, std::size_t) {
                            ...
                            else
                            {
                                if (!m_io)
                                {
                                    LOGHSTRC << "Internal error in handshake: RLPXFrameCoder "
                                                "disappeared.";
                                    m_nextState = Error;
                                    transition();
                                    return;
                                ...
                                {
                                    LOGNETTRC << (m_originated ? "p2p.connect.egress" :
                                                                 "p2p.connect.ingress")
