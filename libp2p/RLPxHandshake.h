...
protected:
    ...
    void setAuthValues(Signature const& sig, Public const& remotePubk, h256 const& remoteNonce,
    ...
    Logger m_hs_trc{createLogger(VerbosityTrace, "net")};

    inline std::string hs_location(const std::string& path) const
    {
        return path.substr(path.find_last_of("/\\") + 1);
    }

#define LOGHSTRC LOG(m_hs_trc) << "[" << hs_location(__FILE__) << ":" << __LINE__ << "] "
};
